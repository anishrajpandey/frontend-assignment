...
const ProductCard = ({
  ...
  return (
    <div className="relative min-h-[380px] h-fit w-[340px] bg-white border-3 hover:scale-105 transition-all shadow-xl rounded-md p-2 mx-1 my-3 cursor-pointer">
      <div
        className="overflow-x-hidden rounded-md h-40 relative"
      ...
      <div
        className="mt-4 pl-2 mb-2 flex justify-between "
        onClick={() => routeToDetailsPage(id)}
      >
        <div>
      ...
      <p
        className="text-sm text-gray-500 p-2 line-clamp-3 "
        onClick={() => routeToDetailsPage(id)}
      >
        {description}
      </p>
      ...
      <div className="flex items-center gap-3 mb-1 pl-3 pt-3 mr-4 group cursor-pointer">
        <button
          className="bg-orange-400 h-10 grid items-center rounded-md w-full text-white hover:bg-orange-700 transition-all "
          onClick={() => routeToDetailsPage(id)}
        >
          {" "}
          Buy Now
        ...
        <svg
          onClick={() => {
            handleCartClick(title, price, image);
          }}
